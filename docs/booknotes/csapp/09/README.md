...
因为空闲块是通过头部中的大小字段隐含地链接着的，所以这种结构称为**隐式空闲链表**。如下图所示，分配器可以通过遍历堆中所有的块，从而间接地遍历整个空闲块的集合。注意，我们需要某种特殊标记的结束块，图中就设置了一个已分配而大小为零的终止头部。
...
![heap_block2](./images/heap_block2.png)
