...
#define PARALLE_PORT_BASE 0x378
...
static ssize_t short_read(struct file *filp, char __user *buf, size_t size, loff_t *ppos)
{
   ...
   unsigned long port = PARALLE_PORT_BASE + (iminor(inode) & 0x0f);
...
static int __init short_init(void)
{
   ...
   if (!request_region(PARALLE_PORT_BASE, DEVICE_NUM, "short"))
   {
   ...
   if (result < 0)
   {
      ...
      release_region(PARALLE_PORT_BASE, DEVICE_NUM);
      return result;
   }
